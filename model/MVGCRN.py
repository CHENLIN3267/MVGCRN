...
from model.MVGCRNCell import MVGCRNCell
class MVGCRM1(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, num_layers=1):
        super(MVGCRM1, self).__init__()
        assert num_layers >= 1, 'At least one DCRNN layer in the Encoder.'
        self.node_num = node_num
        self.input_dim = dim_in
        self.num_layers = num_layers
        self.MVGCRM_cells = nn.ModuleList()
        self.MVGCRM_cells.append(MVGCRNCell(node_num, dim_in, dim_out, cheb_k, embed_dim))
        for _ in range(1, num_layers):
            self.MVGCRM_cells.append(MVGCRNCell(node_num, dim_out, dim_out, cheb_k, embed_dim))
    ...
    def forward(self, x, init_state, node_embeddings):
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):   #如果有两层GRU，则第二层的GGRU的输入是前一层的隐藏状态
                state = self.MVGCRM_cells[i](current_inputs[:, t, :, :], state, [node_embeddings[0][:, t, :, :], node_embeddings[1]])#state=[batch,steps,nodes,input_dim]
    ...
    def init_hidden(self, batch_size):
        init_states = []
        for i in range(self.num_layers):
            init_states.append(self.MVGCRM_cells[i].init_hidden_state(batch_size))
        return torch.stack(init_states, dim=0)      #(num_layers, B, N, hidden_dim)


class MVGCRM2(nn.Module):
    def __init__(self, node_num, dim_in, dim_out, cheb_k, embed_dim, num_layers=1):
        super(MVGCRM2, self).__init__()
        assert num_layers >= 1, 'At least one DCRNN layer in the Encoder.'
        self.node_num = node_num
        self.input_dim = dim_in
        self.num_layers = num_layers
    ...
    def forward(self, x, init_state, node_embeddings):
        ...
        for i in range(self.num_layers):
            ...
            inner_states = []
            for t in range(seq_length):   #如果有两层GRU，则第二层的GGRU的输入是前一层的隐藏状态
                state = self.MVGCRM_cells[i](current_inputs[:, t, :, :], state, [node_embeddings[0], node_embeddings[1]])  #emmmmm这样子是不是可以直接传node_embeddings  #state=[batch,steps,nodes,input_dim]
                # state = self.dcrnn_cells[i](current_inputs[:, t, :, :], state,[node_embeddings[0], node_embeddings[1]])
                inner_states.append(state)   #一个list，里面是每一步的GRU的hidden状态
            output_hidden.append(state)  #每层最后一个GRU单元的hidden状态
            current_inputs = torch.stack(inner_states, dim=1)
...
class MVGCRN(nn.Module):
    def __init__(self, args,adj,adj_poi):
        super(MVGCRN, self).__init__()
        ...
        #self.node_embeddings3 = nn.Parameter(torch.randn(self.num_node, args.embed_dim), requires_grad=True)
        self.T_i_D_emb = nn.Parameter(torch.empty(288, args.embed_dim))
        self.D_i_W_emb = nn.Parameter(torch.empty(7, args.embed_dim))

        self.encoder1 = MVGCRM1(args.num_nodes, args.input_dim, args.rnn_units, args.cheb_k,
                              args.embed_dim, args.num_layers)
        self.encoder2 = MVGCRM2(args.num_nodes, args.input_dim, args.rnn_units, args.cheb_k,
                              args.embed_dim, args.num_layers)
        self.encoder3 = MVGCRM2(args.num_nodes, args.input_dim, args.rnn_units, args.cheb_k,
                              args.embed_dim, args.num_layers)
        self.graph = adj
        self.graph_poi = adj_poi
        #predictor
    ...
    def forward(self, source, i=2):
        ...
        # 第一支路
        node_embeddings1=[node_embedding1,self.node_embeddings1]

        init_state1 = self.encoder1.init_hidden(source.shape[0])   #[2,64,307,64] 前面是2是因为有两层GRU
        output, _ = self.encoder1(source, init_state1, node_embeddings1)      #B, T, N, hidden
        #output = output[:, -1:, :, :]                                   #B, 1, N, hidden
        output = self.dropout1(output[:, -1:, :, :])
        ...
        # 第二支路
        node_embeddings2 = [self.graph, self.node_embeddings1]

        init_state2 = self.encoder2.init_hidden(source.shape[0])   #[2,64,307,64] 前面是2是因为有两层GRU
        output2, _ = self.encoder2(source, init_state2, node_embeddings2)      #B, T, N, hidden
        ...
        #第三支路
        node_embeddings3 = [self.graph_poi, self.node_embeddings1]

        init_state3 = self.encoder3.init_hidden(source.shape[0])   #[2,64,307,64] 前面是2是因为有两层GRU
        output3, _ = self.encoder3(source, init_state3, node_embeddings3)      #B, T, N, hidden
        #output3 = output3[:, -1:, :, :]                                   #B, 1, N, hidden
        output3 = self.dropout3(output3[:, -1:, :, :])
        output3 = self.end_conv3(output3)
