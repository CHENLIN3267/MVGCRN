...
def node_pcc(x, y):
    if len(x.shape)  == 3:
        node = x.shape[1]
        x=x.transpose(1, 2).reshape(-1,node)
        y=y.transpose(1, 2).reshape(-1,node)
    elif len(x.shape)  == 4:
        node = x.shape[2]
        x = x.transpose(2, 3).reshape(-1, node)
        y = y.transpose(2, 3).reshape(-1, node)
    sigma_x = x.std(dim=0)
    sigma_y = y.std(dim=0)
    mean_x = x.mean(dim=0)
    mean_y = y.mean(dim=0)
...
def pcc_torch(x, y, mask_value=None):
    fake_Y, Y = x.reshape(-1), y.reshape(-1)
