...
class MVGCN(nn.Module):
    ...
    def forward(self, x, node_embeddings):
        ...
        #output shape [B, N, C]
        if len(node_embeddings[0].shape) == 3:
            node_num = node_embeddings[0].shape[1]
            supports1 = torch.eye(node_num,device=node_embeddings[0].device)
            filter = self.fc(x)
            nodevec = torch.tanh(torch.mul(node_embeddings[0], filter))  #[B,N,dim_in]
            supports2 = MVGCN.get_laplacian(F.relu(torch.matmul(nodevec, nodevec.transpose(2, 1))), supports1)
            x_g1 = torch.einsum("nm,bmc->bnc", supports1, x)
            x_g2 = torch.einsum("bnm,bmc->bnc", supports2, x)

        elif len(node_embeddings[0].shape) == 2:
            supports1 = torch.eye(node_embeddings[0].shape[1], device=node_embeddings[0].device)
            supports2 = node_embeddings[0]
            x_g1 = torch.einsum("nm,bmc->bnc", supports1, x)
